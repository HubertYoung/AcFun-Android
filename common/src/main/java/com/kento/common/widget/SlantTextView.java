...
public class SlantTextView extends View {
	...
	public SlantTextView( Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes ) {
		super( context, attrs, defStyleAttr, defStyleRes );
		init( attrs );
	}

	public void init( AttributeSet attrs ) {
		TypedArray array = getContext().obtainStyledAttributes( attrs, R.styleable.SlantTextView );

		mTextSize = array.getDimension( R.styleable.SlantTextView_slantedTextSize, mTextSize );
		mTextColor = array.getColor( R.styleable.SlantTextView_slantedTextColor, mTextColor );
		mSlantedLength = array.getDimension( R.styleable.SlantTextView_slantedLength, mSlantedLength );
		mSlantedBackgroundColor = array.getColor( R.styleable.SlantTextView_slantedBackgroundColor, mSlantedBackgroundColor );

		if ( array.hasValue( R.styleable.SlantTextView_slantedText ) ) {
			mSlantedText = array.getString( R.styleable.SlantTextView_slantedText );
		}

		if ( array.hasValue( R.styleable.SlantTextView_slantedMode ) ) {
			mMode = array.getInt( R.styleable.SlantTextView_slantedMode, 0 );
		}
		array.recycle();
	...
	private void drawBackground( Canvas canvas ) {
		...
		if ( w != h ) throw new IllegalStateException( "SlantTextView's width must equal to height" );
	...
	public SlantTextView setText( String str ) {
		mSlantedText = str;
		postInvalidate();
		return this;
	}

	public SlantTextView setText( int res ) {
	...
	public SlantTextView setSlantedBackgroundColor( int color ) {
	...
	public SlantTextView setTextColor( int color ) {
		mTextColor = color;
		mTextPaint.setColor( mTextColor );
		postInvalidate();
		return this;
	...
	/**
	 * @param mode :
	 *             SlantTextView.MODE_LEFT : top left
	 *             SlantTextView.MODE_RIGHT :top right
	 * @return this
	 */
	public SlantTextView setMode( int mode ) {
		if ( mMode > MODE_RIGHT_BOTTOM_TRIANGLE || mMode < 0 ) throw new IllegalArgumentException( mode + "is illegal argument ,please use right value" );
		this.mMode = mode;
		postInvalidate();
		return this;
	}
	...
	public SlantTextView setTextSize( int size ) {
		this.mTextSize = size;
	 ...
	 * @return this
	 */
	public SlantTextView setSlantedLength( int length ) {
