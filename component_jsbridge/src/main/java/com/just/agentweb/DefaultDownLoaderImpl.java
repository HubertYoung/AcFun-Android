...
//                mContext.getPackageManager().resolveActivity(mIntent)
    ...
    private void performDownload(String url, long contentLength, File file) {
        ...
        //并行下载.
        if (isParallelDownload.get()) {
            new DownLoader(new DownLoadTask(NoticationID++, url, this, isForce, enableIndicator, mContext, file, contentLength, mDownLoadMsgConfig, icon == -1 ? R.drawable.ic_file_download_black_24dp : icon)).executeOnExecutor( ExecutorProvider
					.getInstance().provide(), (Void[]) null);
        } else {
            //默认串行下载.
