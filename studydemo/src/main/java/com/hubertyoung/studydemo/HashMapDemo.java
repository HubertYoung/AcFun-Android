...
public class HashMapDemo< K, V > extends AbstractMap< K, V > implements Map< K, V >, Cloneable, Serializable {
		...
		if ( x instanceof Comparable ) {
			...
			if ( ( ts = c.getGenericInterfaces() ) != null ) {
				for (int i = 0; i < ts.length; ++i) {
					if ( ( ( t = ts[ i ] ) instanceof ParameterizedType ) && ( ( p = ( ParameterizedType ) t ).getRawType() == Comparable.class ) && ( as = p.getActualTypeArguments() ) != null && as
							.length == 1 && as[ 0 ] == c ) // type arg is c
						return c;
				}
			}
		}
	...
	final Node< K, V >[] resize() {
		...
		if ( oldCap > 0 ) {
			/**
			 * 	有数据下操作，大于设定最大容量，则放置最大数 2的31次幂-1个数,就只好随你碰撞去吧
			 * 	否则的话且数据大与设定的初始容量，则扩大旧数组数量的二倍
			 */
			if ( oldCap >= MAXIMUM_CAPACITY ) {
				threshold = Integer.MAX_VALUE;
				return oldTab;
			} else if ( ( newCap = oldCap << 1 ) < MAXIMUM_CAPACITY && oldCap >= DEFAULT_INITIAL_CAPACITY ) newThr = oldThr << 1; // double threshold
		} else if ( oldThr > 0 ) // 赋值过阈值，直接设定阈值
			newCap = oldThr;
		else {               // 未使用直接使用默认值
			newCap = DEFAULT_INITIAL_CAPACITY;
			newThr = ( int ) ( DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY );
		}
		if ( newThr == 0 ) {//无数据的情况下，旧数组设置过阈值条件，手动设置新数组的阈值
		...
		if ( oldTab != null ) {
			for (int j = 0; j < oldCap; ++j) {
				...
				if ( ( e = oldTab[ j ] ) != null ) {
					oldTab[ j ] = null;//手动回收
					if ( e.next == null ) newTab[ e.hash & ( newCap - 1 ) ] = e;//当前索引无数据，直接赋值
					else if ( e instanceof TreeNode ) ( ( TreeNode< K, V > ) e ).split( this, newTab, j, oldCap );//红黑树操作过，直接分割
					else { // 当前是正常链表结构
						//一堆初始化
						...
						Node< K, V > next;
						do {
							next = e.next;//拿到链表指向的节点
							if ( ( e.hash & oldCap ) == 0 ) {// 是原索引
