...
public class DownLoader extends AsyncTask<Void, Integer, Integer> implements Observer {
    ...
    private void doCallback(Integer code) {
        ...
        if (code > 200) {
            mDownLoadResultListener.error(mDownLoadTask.getFile().getAbsolutePath(), mDownLoadTask.getUrl(), DownLoadMsg.getMsgByCode(code), this.e == null ? new RuntimeException("download fail ， cause:" + DownLoadMsg

					.getMsgByCode(code)) : this.e);
        } else {
