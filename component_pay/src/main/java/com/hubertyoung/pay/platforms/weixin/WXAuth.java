...
public class WXAuth extends WXBase implements IAuthorize {
	...
	public void authorize( @NonNull OnCallback< String > callback ) {
		...
		if ( !mApi.isWXAppInstalled() ) {
			mCallback.onFailed( mActivity, ResultCode.RESULT_FAILED, "ddpsdk_error_operate wechat" );
			return;
		}
		...
		if ( !this.mApi.sendReq( req ) ) {
			callback.onFailed( mActivity, -1, "sendReq fail" );
...
//				String fromHashMap = new Hashon().fromHashMap( data );
				getToken( resp.code );
//				if ( mCallback != null ) {
//					mCallback.onSucceed( mActivity, fromHashMap );
//				}
				break;

			case BaseResp.ErrCode.ERR_USER_CANCEL:      //授权取消
				if ( mCallback != null ) {
					mCallback.onFailed( mActivity, resp.errCode, resp.errStr );
				}
				break;
...
//						token.setResult(result);
					...
					if ( mCallback != null ) {
						mCallback.onFailed( mActivity, -1, e.getMessage()
															.toString() );
					}
				}
			}
