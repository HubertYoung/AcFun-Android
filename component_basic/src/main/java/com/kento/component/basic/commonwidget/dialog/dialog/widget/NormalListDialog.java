package com.kento.component.basic.commonwidget.dialog.dialog.widget;
...
public class NormalListDialog extends BaseDialog< NormalListDialog > {
    ...
    //item是否进入有动画  true：有动画  false：无动画
    private boolean itemIsAnim = true;
    ...
    public NormalListDialog( Context context, ArrayList< DialogMenuItem > baseItems, boolean itemIsAnim ) {
        super( context );
        mContents.addAll( baseItems );
        this.itemIsAnim = itemIsAnim;
        screenSize = ScreenUtils.getScreenSize( context, true );
        if ( itemIsAnim ) {
            init();
        }
    }

    public NormalListDialog( Context context, String[] items, boolean itemIsAnim ) {
        super( context );
    ...
    public NormalListDialog( Context context, BaseAdapter adapter, boolean itemIsAnim ) {
        super( context );
        mAdapter = adapter;
        screenSize = ScreenUtils.getScreenSize( context, true );
        this.itemIsAnim = itemIsAnim;
    ...
    public View onCreateView() {
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams( ( screenSize[ 0 ] * 3 ) / 4,
