package com.kento.component.basic.commonwidget.filter.view;
...
import com.kento.component.basic.commonwidget.filter.adapter.MenuAdapter;
...
public class FixedTabIndicator extends LinearLayout {
	...
	private int mLineColor = 0xFFcccccc;

	private int mTabTextSize = 13;// 指针文字的大小,sp
	private int mTabDefaultColor = R.color.text_color_gray78;// 未选中默认颜色
	private int mTabSelectedColor = R.color.text_color_normal;// 指针选中颜色
	...
	private void switchTab( int pos ) {
		...
		if ( mLastIndicatorPosition == pos ) {
			// 点击同一个条目时
			tv.setTextColor( ContextCompat.getColor( getContext(), level == 0 ? mTabSelectedColor : mTabDefaultColor ) );
		...
		//highLightPos(pos);
		tv.setTextColor( ContextCompat.getColor( getContext(), mTabSelectedColor ) );
	...
	public void resetPos( int pos ) {
		TextView tv = getChildAtCurPos( pos );
		tv.setTextColor( ContextCompat.getColor( getContext(), mTabDefaultColor ) );
