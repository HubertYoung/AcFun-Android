 ...
 * @since:V1.0
 * @desc:com.hubertyoung.common.commonwidget.sidebar
 */
...
//        mLetters = Arrays.asList(context.getResources().getStringArray( R.array.waveSideBarLetters));
		...
		mTextColorChoose = context.getResources()
								  .getColor( android.R.color.white );
		mTextSize = context.getResources()
						   .getDimensionPixelSize( R.dimen.text_size10 );
		mLargeTextSize = context.getResources()
								.getDimensionPixelSize( R.dimen.text_size32 );
		mPadding = context.getResources()
						  .getDimensionPixelSize( R.dimen.text_size20 );
		if ( attrs != null ) {
	...
	private void drawChooseText( Canvas canvas ) {
		if ( mChoose != -1 ) {
			...
			if ( mRatio >= 0.9f ) {
				...
				mTextPaint.setTextSize( isChinese( target ) ? mLargeTextSize / 2 : mLargeTextSize );
	...
	private void drawBallPath( Canvas canvas ) {
		//x轴的移动路径
		mBallCentreX = ( mWidth + mBallRadius ) - ( 2.0f * mRadius + 2.0f * mBallRadius ) * mRatio * 1.2f;
	...
	private boolean isChinese( String s ) {
		for (int i = 0; i < s.length(); i++) {
			...
			Matcher matcher = pattern.matcher( String.valueOf( s.charAt( i ) ) );
			if ( matcher.matches() ) {
				return true;
			}
		}
