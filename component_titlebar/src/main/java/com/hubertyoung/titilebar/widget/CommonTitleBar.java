package com.hubertyoung.titilebar.widget;
...
public class CommonTitleBar extends RelativeLayout implements View.OnClickListener {
	...
	private void loadAttributes( Context context, AttributeSet attrs ) {
		PADDING_5 = DisplayUtil.dip2px( 5 );
		PADDING_12 = DisplayUtil.dip2px( 12 );
		...
		titleBarHeight = ( int ) array.getDimension( R.styleable.CommonTitleBar_titleBarHeight, DisplayUtil.dip2px( 44 ) );
		...
		bottomShadowHeight = array.getDimension( R.styleable.CommonTitleBar_bottomShadowHeight, DisplayUtil.dip2px( 0 ) );
		...
		if ( leftType == TYPE_LEFT_TEXTVIEW ) {
			...
			leftTextSize = array.getDimension( R.styleable.CommonTitleBar_leftTextSize, DisplayUtil.dip2px( 16 ) );
		...
		if ( rightType == TYPE_RIGHT_TEXTVIEW ) {
			...
			rightTextSize = array.getDimension( R.styleable.CommonTitleBar_rightTextSize, DisplayUtil.dip2px( 16 ) );
		...
		if ( centerType == TYPE_CENTER_TEXTVIEW ) {
			...
			centerTextSize = array.getDimension( R.styleable.CommonTitleBar_centerTextSize, DisplayUtil.dip2px( 18 ) );
			...
			centerSubTextSize = array.getDimension( R.styleable.CommonTitleBar_centerSubTextSize, DisplayUtil.dip2px( 11 ) );
		...
		array.recycle();
		mMaxHight = titleBarHeight + BarUtils.getStatusBarHeight();
	}
	...
	private void initGlobalViews( Context context ) {
		...
		boolean transparentStatusBar = OSUtil.isMIUI6Later() || OSUtil.isFlyme() || ( OSUtil.isOppo() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ) || Build.VERSION.SDK_INT >= Build
				.VERSION_CODES.M;
...
//			BarUtils.setStatusBarColor( getWindow(), statusBarColor, false );
		}
		...
		// 计算主布局高度
		if ( showBottomLine ) {
			mainParams.height = titleBarHeight - Math.max( 1, DisplayUtil.dip2px( 0.4f ) );
		} else {
		...
		if ( showBottomLine ) {
			...
			LayoutParams bottomLineParams = new LayoutParams( MATCH_PARENT, Math.max( 1, DisplayUtil.dip2px( 0.4f ) ) );
			...
			LayoutParams bottomShadowParams = new LayoutParams( MATCH_PARENT, DisplayUtil.dip2px( bottomShadowHeight ) );
	...
	private void initMainCenterViews( Context context ) {
		if ( centerType == TYPE_CENTER_TEXTVIEW ) {
			...
			// 设置跑马灯效果
			tvCenter.setMaxWidth( ( int ) ( DisplayUtil.getScreenPixelSize( context )[ 0 ] * 3 / 5.0 ) );
			...
			int progressWidth = DisplayUtil.dip2px( 18 );
			...
			// 设置边距
			centerParams.topMargin = DisplayUtil.dip2px( 7 );
			centerParams.bottomMargin = DisplayUtil.dip2px( 7 );
			...
			ivSearch.setId( BarUtils.generateViewId() );
			ivSearch.setOnClickListener( this );
			int searchIconWidth = DisplayUtil.dip2px( 15 );
			...
			ivVoice.setId( BarUtils.generateViewId() );
			...
			etSearchHint.setTextSize( TypedValue.COMPLEX_UNIT_PX, DisplayUtil.dip2px( 14 ) );
...
//            }
	...
	protected void onMeasure( int widthMeasureSpec, int heightMeasureSpec ) {
		...
		if ( heightMode == MeasureSpec.EXACTLY ) {
			heightSize = heightSize <= mMaxHight ? heightSize : ( int ) mMaxHight;
		}
	...
	private void setUpImmersionTitleBar() {
		...
		// 设置状态栏背景透明
		BarUtils.setStatusBarTranslucent( window, true );
//		BarUtils.setPaddingSmart( this );
		// 设置图标主题
		BarUtils.statusBarLightMode( window, statusBarMode == 0 );
		// 解决部分手机输入框被键盘遮挡的问题
...
//		BarUtils.setStatusBarColor( this.getWindow(), color, false );
...
//			BarUtils.setPaddingSmart( this );
//		} else {
//			BarUtils.removePaddingSmart( this );
//		}
		if ( viewStatusBarFill != null ) {
...
//		BarUtils.setStatusBarTranslucent( window, true );
		if ( statusBarMode == 0 ) {
			statusBarMode = 1;
			BarUtils.statusBarLightMode( window, false );
		} else {
			statusBarMode = 0;
			BarUtils.statusBarLightMode( window, true );
		}
	}
	...
	public void showSoftInputKeyboard( boolean show ) {
		if ( centerSearchEdiable && show ) {
			...
			KeyBordUtil.popSoftKeyboard( getContext(), etSearchHint, show );
		} else {
			KeyBordUtil.popSoftKeyboard( getContext(), etSearchHint, show );
		}
	}
