...
import com.zhy.autolayout.widget.R;
...
public class AutoUtils {
	...
	public static void auto( View view, int attrs, int base ) {
		...
		if ( autoLayoutInfo != null ) autoLayoutInfo.fillAttrs( view );
	}
	...
	public static float getPercentHeight1px() {
		...
		int designHeight = AutoLayoutConifg.getInstance()
										   .getDesignHeight();
		if ( AutoLayoutConifg.getInstance()
							 .isFullScreen() ) {
			screenHeight = AutoLayoutConifg.getInstance()
										   .getScreenWidth() * designHeight / designWidth;
		}
